    ...
    <div class="aircon-entry" data-aircon-id="${id}">
        ...
        <div class="form-group">
              <label>Issue / Problem</label>
                   <textarea id="repairProblem-${id}"
                              class="repair-problem form-control"
                              rows="3"
                              placeholder="Describe the issue with this AC..."></textarea>
        </div>
...
function getEntrySignature(entry) {
    ...
    const repairProblem = document.getElementById(`repairProblem-${id}`)?.value?.trim() || '';

    return `${acType}|${service}|${tier}|${brand}|${repairProblem}`;
}
...
function setupNavigation() {
    ...
    nextBtn.addEventListener('click', () => {
        ...
        for (const entry of entries) {
            ...
            // NEW FIELD
            const repairProblem = document.getElementById(`repairProblem-${id}`);
...
function renderAirconSummary() {
    ...
    entries.forEach((entry, index) => {
        ...
        const repairProblem =
            document.getElementById(`repairProblem-${id}`)?.value?.trim() || '';
...
async function submitBooking() {
    ...
    entries.forEach(entry => {
        ...
        // NEW fields
        const repairProblemInput = document.getElementById(`repairProblem-${id}`);
        ...
        payload.sbitems.push({
            ...
            // NEW fields
            repair_problem_desc: repairProblemInput?.value?.trim() || null,
    ...
    try {
        const res = await fetch("/api/service-bookings/repair", {
            method: "POST",
